...
parser.add_argument('--nsample', type=int, default=1, help='number of samples')
...
def make_gifs(x, idx, name):
    ...
    min_over_t_errL1 = np.zeros((opt.batch_size, nsample, opt.n_future))
    best_time = np.zeros((opt.batch_size, nsample, opt.n_future))
    ...
    for s in range(nsample):
        ...
        for i in range(1, opt.n_eval):
            ...
            if i < opt.n_past:
                ...
                all_gen[s].append(x_in)
        _, ssim[:, s, :], psnr[:, s, :], min_over_t_errL1[:, s, :], best_time[:, s, :] = utils.eval_seq(gt_seq, gen_seq)

    progress.finish()
    utils.clear_progressbar()

    return min_over_t_errL1, best_time
...
for i in range(0, len(test_data), opt.batch_size):
    # # plot train
    ...
    a, b = make_gifs(test_x, i, 'test')
    if 'min_over_t_errL1' in locals():
        min_over_t_errL1 = np.concatenate((min_over_t_errL1, a), axis=0)
        best_time = np.concatenate((best_time, b), axis=0)
    else:
        min_over_t_errL1 = a
        best_time = b
    print(i)
...
print("Min_l1: ")
print(min_over_t_errL1.mean(axis=0))
print("match_step: ")
print(best_time.mean(axis=0))
print(min_over_t_errL1.std(axis=0))
print(best_time.std(axis=0))
